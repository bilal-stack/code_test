...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cUser->is('customer')) {
            $jobs = $cUser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average',
                'language', 'feedback')->whereIn('status', ['pending', 'assigned', 'started'])
                ->orderBy('due', 'asc')->get();
            $userType = 'customer';
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cUser->is('customer')) {
            $jobs = $cUser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average',
                'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24',
                'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);

            $userType = 'customer';

//            TODO: I did not change noramlJobs to normalJobs cause we have to change it from frontend too.
//            I don't know if it is a typo or some type of job.
            return [
                'emergencyJobs' => $emergencyJobs,
                'noramlJobs'    => [],
                'jobs'          => $jobs,
...
//     I am assuming there is only two types of user. if not then there will be elseif.
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if (in_array('normal', $data['job_for']) && in_array('certified', $data['job_for'])) {
                ...
                // need to fix typo 'certified_in_helth' but i don't know if the typo is coming from database or just here.
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobEnd($post_data = array())
    {
        ...
        // can create private function for sending emails based on parameters provided, this will drastically improve duplications.
        $name = $user->name;
    ...
    // why there is a optional parameter before required?
    /**
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $phoneJobMessageTemplate = trans('sms.phone_job', ['date' => $date, 'time' => $time, 'duration' => $duration,
            'jobId' => $jobId]);

        $physicalJobMessageTemplate = trans('sms.physical_job', ['date' => $date, 'time' => $time, 'town' => $city,
            'duration' => $duration, 'jobId' => $jobId]);

        // This shouldn't be feasible job, so no handling of this edge case
        $message = '';
    ...
    /**
     * Change status withdraw after 24 hours.
     *
     * @param $job
     * @param $data
     * @return bool
    ...
    /**
     * sends change translator notification
     *
     * @param $job
     * @param $current_translator
    ...
    /**
     * Accept the job with the job id
     *
     * @param $job_id
     * @param $cuser
     *
...
//                Event::fire(new JobWasCanceled($job));
    ...
    /**
     * Gets the potential jobs for paid,rws,unpaid translators
     *
     * @param $cuser
     *
     * @return mixed
